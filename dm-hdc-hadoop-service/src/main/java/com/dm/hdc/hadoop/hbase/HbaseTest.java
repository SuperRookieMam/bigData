package com.dm.hdc.hadoop.hbase;
...
public class HbaseTest {
  ...
  /*  @Autowired
    ...
    public List<TableDescriptor> getTableDescriptor(String...tablename){

        Admin admin = null;
        try {
            admin = ConnectionFactory.createConnection(configuration).getAdmin();
            Admin finalAdmin = admin;
            return Arrays.stream(tablename).map(ele-> {
                try {
                    return finalAdmin.getDescriptor(TableName.valueOf(ele));
                } catch (IOException e) {
                    e.printStackTrace();
                }
                return null;
